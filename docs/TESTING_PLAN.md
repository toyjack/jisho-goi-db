...
- [x] **搜索与结果流程（8/9 模块已完成，2026-07-23）**：`tests/e2e/database-modules.contract.ts` + `database-modules.spec.ts` 落地了参数化契约测试，覆盖 `Jiruisho`、`Racvyoxv`、`Bunmei`、`Gyokuhentaizen`、`Hzwm`、`Kwrs`、`Wakunnoshiori`、`TsjWakun` 共 8 个模块（全部只读模块）。断言点：搜索命中→详情页字段展示→无结果不崩溃，对当前生产数据库直接可跑（只读，不涉及写入）。
  - 查询策略需按模块区分：多数模块的搜索字段是 `contains` 模糊匹配，空字符串等于全匹配，可安全作为"至少命中一条结果"的通用查询；但 `Gyokuhentaizen` 的 `entry` 是**精确匹配**（`db/gyokuhentaizen.ts:36`），传空字符串会变成 `entry: ""` 的精确查询而非"不过滤"，因此该模块的 `knownQuery` 是空对象而不是 `{ entry: "" }`——契约表中已注明。
  - **`TsjWakun` 是唯一 Supabase 直连模块**（不经过 Prisma，见 `db/tsj_wakun.ts`），搜索字段名是 `entry_text` 而非 `entry`。为此契约表新增了 `noResultQueryField` 字段，让"无结果"用例按模块使用各自真实存在的字段名，避免对 Supabase 的 `.ilike()` 传入不存在的列名而报错。
  ...
  - 扩展到 `TsjWakun` 时还发现本地 `.env` 一度缺少 `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`，导致该模块本地开发环境完全不可用（不只是测试）；已由用户补齐 `.env` 解决。
  - 剩余只有写入型的 `Jiruisho-Chushaku` 未加入契约表，受"确定测试专用环境与数据"决策影响暂缓。
