...
4. 异体字检索开关已经接好但还没法做真实效果对比——本地 API 的 TSJ 数据集 active 版本目前只有 1 条记录，等完整数据集重新发布后再验证。
5. "信息架构评审"排最后，依赖前面几项落地后才有实际内容可评。
6. 正式 API 服务器地址等用户部署后告知，届时替换 `.env` 里的 `HDIC_API_BASE_URL`。
7. `tests/e2e/database-modules.contract.ts` 里 `TsjWakun` 的 `knownQuery` 是 `{ entry_text: "一" }`（新 API 的 `/search` 要求 `q` 必填，不能再用空字符串"全件ヒット"的旧技巧）——**注意**：这条查询是针对之前 20017 条记录那个版本验证过的，如果 active 版本长期停留在这种小样本测试数据，contract 测试会因为搜不到"一"而失败，等完整数据集重新发布后建议重新跑一遍确认。
